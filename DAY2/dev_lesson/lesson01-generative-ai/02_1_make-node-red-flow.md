# 生成AIを使ってNode-REDの処理を書いてみる

生成 AI をプロトタイピングに役立たせてみましょう。

この授業では Node-RED を使ったローコード開発をしてきましたが、生成 AI でさらに開発を便利にすることができます。
...
### 今回はこれと同様のことを Effibot を使って進めてみます。

`Function GPTノード`は API キーを発行して利用するのですが、授業で全員でやるとトラブルがある可能性もあるので`Function GPTノード`は使わずに、同様のことを Effibot を使ってやってみます。
...
`changeノード`は仮のセンター値として200を設定してみましょう。最後に`debugノード`も接続してください。
...
`injectノード`の実行してみましょう。200というセンサーデータが`functionノード`に送られ、内部では **Effibot が書いてくれたコードが処理をしてセンサーデータが300以下なのかどうかを判定**してくれています。

> <img src="https://i.gyazo.com/b319f1c135968a407ac1f1dae8238e23.png" width="400px" />

画像のように `debugノード`に200というデータが出れば成功です。

- 上手くいったら、`chagneノード`の値を400などに変更して再度試してください。

おそらくこれは反応がないと思います。`functionノード`内の処理は300以下のときのみ反応させたかったので正常な挙動です。
