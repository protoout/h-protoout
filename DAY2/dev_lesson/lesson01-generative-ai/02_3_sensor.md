...
> 最終的につくりたいものは、距離センサーとスピーカーを連携させる仕組みです。
...
`設定ノード`の初期化処理と`obniz repeatノード`に中に以下のコードを書きます。詳細はこちらの[マニュアル](https://zenn.dev/protoout/books/07_node-red-obniz/viewer/sensor-distance-hcsr04)でも紹介しています。
...
そして、`設定ノード`の初期化処理と`obniz functionノード`に中に以下のコードを書きます。詳細はこちらの[マニュアル](https://zenn.dev/protoout/books/07_node-red-obniz/viewer/actuator-speaker)でも紹介しています。
...
- Effibot にプロンプトを指定して聞くと`functionノード`で使えるコードが出力されて、機能追加なども日本語の指示で行えます。
- **人間は日本語で指示を出すことでコードを書くプログラミングがノーコードツールのように扱えるよう**になってきています。
- `injectノード`,`changeノード`,`templateノード`でセンサーを繋がなくてもエミュレートができます。
    - これはチームで分担する時に使えます。
- obnizノードの`設定ノード`は2つのセンサーやアクチュエーターなどを組み合わせるときは上書きではなく更新なので注意しましょう。

### 3-6. おまけ: 時間あれば考えてみよう

**Q. この章で作った仕組みから企画を考えてみましょう。**

- ヒント: 距離センサーは色々な応用ができます。[距離センサ活用術](https://qiita.com/wicket/items/dcc1578f3313046c6842)にたくさん応用例や考え方が載っています。

- ヒント: 例えば僕はこんなの考えてみました。
